...
/*   Updated: 2024/11/04 14:06:13 by mosokina         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (data->wf_pos == 'u')
		mlx_put_image_to_window(data->mlx, data->win, data->imgs.wf_up_img,
			(data->map.wf.x * IMG_W), (data->map.wf.y * IMG_H));
	else if (data->wf_pos == 'd')
		mlx_put_image_to_window(data->mlx, data->win, data->imgs.wf_down_img,
			(data->map.wf.x * IMG_W), (data->map.wf.y * IMG_H));
	else if (data->wf_pos == 'l')
		mlx_put_image_to_window(data->mlx, data->win, data->imgs.wf_lft_img,
			(data->map.wf.x * IMG_W), (data->map.wf.y * IMG_H));
	else if (data->wf_pos == 'r')
		mlx_put_image_to_window(data->mlx, data->win, data->imgs.wf_rght_img,
			(data->map.wf.x * IMG_W), (data->map.wf.y * IMG_H));
}
